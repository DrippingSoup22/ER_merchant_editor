...
`internal/savefile/` — **package `savefile`** (importable; the GUI calls
...
the patched blob). `cmd/shopwrite` is the thin CLI wrapper (same
...
```
go run ./cmd/shopwrite -save <in.dat> -out <out.dat> -entry ShopLineupParam.param -edits <edits.json>
```
...
Prebuilt CLI binaries: `scripts/build.sh` → `dist/shopwrite/<goos>-<goarch>/`.
...
exactly — no new schema-computation logic). `internal/ui/gio/state.go`'s
...
- 2026-08-02 (first recompression attempt): `TestApplyManyDistinctRowsInOneWrite`
  ...
  with capacity to spare. Full offline test suite (`go test ./...`)
