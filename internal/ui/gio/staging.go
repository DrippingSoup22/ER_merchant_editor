...
// savefile.Edit records for write-back. All methods run on the UI goroutine
...
// BuildEdits flattens the staged edits into savefile.Edit records, exactly like
...
func (s *State) BuildEdits() []savefile.Edit {
	return s.Session.BuildEdits()
}
...
// written.
type equipParamRef = application.EquipParamRef
...
type equipParamTarget = application.EquipParamTarget
...
func equipParamRefForEdit(row *catalog.Row, e *RowEdit) (ref equipParamRef, price int64, ok bool) {
	return application.EquipParamRefForEdit(row, e)
}
...
func (s *State) computeEquipParamTargets() (targets map[equipParamRef]*equipParamTarget, rowRefs map[int64]equipParamRef, err error) {
	return s.Session.ComputeEquipParamTargets()
}

// BuildEquipParamEdits flattens computeEquipParamTargets' per-item targets
// into the savefile.Edit records combinedApplyWorker's sellValue pipeline
// stage writes, one entry per distinct EquipParam* table touched.
func (s *State) BuildEquipParamEdits() (map[string][]savefile.Edit, error) {
	return s.Session.BuildEquipParamEdits()
}
...
func (s *State) SuggestedOutPath() string {
	return s.Session.SuggestedOutPath()
}
