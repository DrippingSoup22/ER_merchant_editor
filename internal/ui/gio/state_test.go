...
func TestCombinedApplyWorkerMergesItemAndFlagEdits(t *testing.T) {
	...
	applyPendingForTest(t, s, outPath)
	...
	states, err := character.LockStates(data, 7, []*catalog.Row{rows2[flagRowID]})
	if err != nil {
		t.Fatal(err)
	}
	if !states[flagRowID] {
...
func TestCombinedApplyWorkerMergesBellBearingEdits(t *testing.T) {
	...
	states, err := character.FlagStates(data, 7, []uint32{flagID})
	if err != nil {
		t.Fatal(err)
	}
	if !states[flagID] {
...
func TestCombinedApplyWorkerSellValueRisesWhenSafeCappedBySiblings(t *testing.T) {
	...
	// internal/savefile/apply.go, docs/MERCHANT_DATA.md) -- an orthogonal,
	...
	for i, c := range candidates {
		...
		applyPendingForTest(t, trial, out1)
