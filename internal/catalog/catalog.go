...
// the former Python save scanner and catalog; write-back is delegated to savefile.Apply
...
type Catalog struct {
	...
	shopSchema   *savefile.ShopSchema
	mtrlSchema   *savefile.ShopSchema
...
func New() (*Catalog, error) {
	...
	if c.shopSchema, err = savefile.LoadShopSchema(); err != nil {
		return nil, err
	}
...
func (c *Catalog) LoadSave(path string) error {
	if _, err := savefile.LoadRegulation(path); err != nil {
		return err
	}
	c.savePath = path
	c.rows = nil
...
func (c *Catalog) ShopRows() ([]*Row, error) {
	...
	reg, err := savefile.LoadRegulation(path)
	if err != nil {
		return nil, err
	}
	_, blob, _, paramRows, err := savefile.LoadParamRows(reg.BND4, shopEntry)
	if err != nil {
		return nil, err
	}
...
// ApplyEdits validates the edits, then calls savefile.Apply in-process. On
...
func (c *Catalog) ApplyEdits(edits []savefile.Edit, outPath string) (*savefile.Summary, error) {
	if len(edits) == 0 {
	...
	summary, err := savefile.Apply(path, outAbs, shopEntry, edits)
	if err != nil {
