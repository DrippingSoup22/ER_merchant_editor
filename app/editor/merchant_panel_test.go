...
func TestMerchantViewsKeepEditingStableAndPreviewGameOrder(t *testing.T) {
	...
	s := &State{Catalog: c, MerchantRows: rows, PendingEdits: map[int64]*RowEdit{}}
	...
	reloaded := &State{Catalog: c, MerchantRows: saved, PendingEdits: map[int64]*RowEdit{}, merchantGamePreview: true}
...
func TestCellBorder(t *testing.T) {
	...
	newState := func() *State { return &State{PendingEdits: map[int64]*RowEdit{}} }
...
func TestFormActionsRowFitsModalWidth(t *testing.T) {
	...
	s := &State{
		...
		lastMerchant: "Twin Maiden Husks",
		SelectedChar: -1, // drives the longest ("All Characters") labels
	}
	rows := []*catalog.Row{
