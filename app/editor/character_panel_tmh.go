...
// and app/charunlock.BellBearing. Depends only on shared helpers living in
...
func bellBearingGroupRank(b charunlock.BellBearing) int {
	switch {
...
func bellBearingShopSortKey(b charunlock.BellBearing) string {
...
func sortBellBearingsByFamily(bb []charunlock.BellBearing) {
...
func splitTMHBearingColumns(bearings []charunlock.BellBearing) (left, right []charunlock.BellBearing) {
...
// charunlock.BellBearingsForUI() into the 3 sections
...
//   - npcBearings ("NPC Bell Bearings"): charunlock.BellBearingsForUI()
...
func tmhFlagSections(rows []*catalog.Row) (tmhBearings, otherItems []tmhOtherItem, npcBearings []charunlock.BellBearing) {
	byFlag := make(map[int64]charunlock.BellBearing, len(charunlock.BellBearings))
	for _, b := range charunlock.BellBearings {
	...
	npcBearings = append(npcBearings, charunlock.BellBearingsForUI()...)
...
// to scan.
func (s *State) layoutBellBearingRow(gtx layout.Context, th *material.Theme, b charunlock.BellBearing) layout.Dimensions {
...
// flag matches a charunlock.BellBearing entry ("TMH Bell Bearings" in
...
// label used to use).
func (s *State) layoutFlagRowNamed(gtx layout.Context, th *material.Theme, g flagGroup, bb charunlock.BellBearing) layout.Dimensions {
