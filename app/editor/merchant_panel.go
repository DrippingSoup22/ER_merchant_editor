...
// is no longer a border color (2026-07-29) -- see widgets.IconCell.Locked's
...
// (widgets.BadgeScale) so the Shop Editor's cell-size slider actually makes
...
func (s *State) rowPriceFooter(th *material.Theme, row *catalog.Row) layout.Widget {
	...
	scale := widgets.BadgeScale(s.merchantCellSize())
