...
// via app/cmd/charunlock, see docs/CHAR_UNLOCK.md).
...
func TestEnsureMerchantGatedMatchesCharunlock(t *testing.T) {
	...
	// Cross-check EVERY entry directly against app/charunlock. Total/unlocked
...
func TestEniaReadOnlyLockDisplay(t *testing.T) {
	...
	states, err := charunlock.LockStates(s.charSaveData, s.SelectedChar, eniaRows)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSelectFlagMerchantPopulatesRows(t *testing.T) {
	...
	if target == twinMaidenHusksMerchantName {
		wantRows -= len(charunlock.BellBearingsForUI())
	}
...
func TestBellBearingSectionOnlyForTMH(t *testing.T) {
	...
	if len(s.bellBearingState) != len(charunlock.BellBearingsForUI()) {
		t.Errorf("bellBearingState has %d entries, want %d (BellBearingsForUI)", len(s.bellBearingState), len(charunlock.BellBearingsForUI()))
	}
...
// charunlock.BellBearingsForUI() entry must appear exactly once in
...
func TestLayoutTMHFlagsSectionCoversEveryEntry(t *testing.T) {
	...
	if got, want := len(npcBearings), len(charunlock.BellBearingsForUI()); got != want {
		t.Fatalf("len(npcBearings) = %d, want %d (charunlock.BellBearingsForUI(), unsplit)", got, want)
	}
...
func TestNamedBellBearingShopSequence(t *testing.T) {
	...
	bearings := charunlock.BellBearingsForUI()
...
func TestPendingFlagCountCollapsesSharedUnlockFlag(t *testing.T) {
	s := &State{
		PendingEdits:            map[int64]*RowEdit{900: {}},
		PendingFlagEdits:        map[int]map[int64]bool{3: {101: true, 102: true, 103: true}},
		PendingBellBearingEdits: map[int]map[uint32]bool{3: {7001: true}},
		charFlagFlag:            map[int64]int64{101: 500, 102: 500, 103: 501},
	}
...
func TestToggleEveryMerchantUnlocksRestoresExactlyItsOwnChanges(t *testing.T) {
	...
	s := &State{
		SelectedChar:            1,
		charDataPath:            "fixture",
		gatedCachePath:          "fixture",
		gatedCacheChar:          1,
		charFlagState:           map[int64]bool{101: false, 102: true},
		tmhBellCommitted:        map[uint32]bool{},
		PendingFlagEdits:        map[int]map[int64]bool{1: {102: false}},
	...
	firstBearing := charunlock.BellBearingsForUI()[0].FlagID
