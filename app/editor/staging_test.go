...
func TestSwapClearsDisplayOverrides(t *testing.T) {
	s := &State{PendingEdits: map[int64]*RowEdit{}}
	row := noteRow()
...
func TestStageItemSwapCapturesMerchant(t *testing.T) {
	s := &State{PendingEdits: map[int64]*RowEdit{}, lastMerchant: "Merchant Kale"}
	row := noteRow()
	s.stageItemSwapCore(s.PendingEdits, row, sellableItem("Rune Arc", 190))
...
func TestStageItemSwapCoreAppliesPickLevel(t *testing.T) {
	...
	s := &State{PendingEdits: map[int64]*RowEdit{}, Catalog: cat, PickLevel: 7}
...
func TestStageItemSwapCoreClampsPickLevelToItemMax(t *testing.T) {
	...
	s := &State{PendingEdits: map[int64]*RowEdit{}, Catalog: cat, PickLevel: 99}
...
func TestStageItemSwapCoreNonWeaponIgnoresPickLevel(t *testing.T) {
	...
	s := &State{PendingEdits: map[int64]*RowEdit{}, Catalog: cat, PickLevel: 5}
	row := noteRow()
...
func TestStageItemLevelOnUnswappedRow(t *testing.T) {
	...
	zwei := findItem(t, cat, "Zweihander")
	s := &State{PendingEdits: map[int64]*RowEdit{}, Catalog: cat}
...
func TestStageItemLevelAdjustsExistingSwap(t *testing.T) {
	...
	moonveil := findItem(t, cat, "Moonveil")
	s := &State{PendingEdits: map[int64]*RowEdit{}, Catalog: cat, PickLevel: 3}
	row := noteRow()
	row.RowID = 3
	s.stageItemSwapCore(s.PendingEdits, row, moonveil)
...
func TestGateActionSpecEniaDisabled(t *testing.T) {
	s := &State{PendingEdits: map[int64]*RowEdit{}, lastMerchant: eniaMerchantName, SelectedChar: -1}
...
// effect of a price/item edit -- only app/charunlock (one character's own
