...
func decodeMagnifications(t *testing.T) []float32 {
	t.Helper()
	reg, err := shopwrite.LoadRegulation(fixtureSave)
	if err != nil {
		t.Fatal(err)
	}
	entry, err := shopwrite.FindBND4Entry(reg.BND4, "ShopLineupParam.param")
	if err != nil {
		t.Fatal(err)
	}
	blob := reg.BND4[entry.Offset : entry.Offset+entry.Size]
	header, err := shopwrite.ParseParamHeader(blob)
	if err != nil {
		t.Fatal(err)
	}
	prows, err := shopwrite.IterParamRows(blob, header)
	if err != nil {
		t.Fatal(err)
	}
	schema, err := shopwrite.LoadShopSchema()
	if err != nil {
		t.Fatal(err)
	}
