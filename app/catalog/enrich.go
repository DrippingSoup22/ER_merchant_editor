...
func loadMtrlSchema() (*shopwrite.ShopSchema, error) {
	raw, err := data.FS.ReadFile("equip_mtrl_set_schema.json")
	if err != nil {
		return nil, err
	}
	return shopwrite.ParseSchema(raw)
}
...
func loadMtrlContext(bnd4 []byte, schema *shopwrite.ShopSchema) (*mtrlContext, error) {
	_, blob, _, rows, err := shopwrite.LoadParamRows(bnd4, "EquipMtrlSetParam.param")
	if err != nil {
		return nil, err
	}
...
func resolveMaterials(mtrlID int64, mtrl *mtrlContext, itemsByID map[int64]*Item) ([]Material, error) {
	...
	fields, err := shopwrite.DecodeRowFields(mtrl.blob, dataOffset, mtrl.schema)
	if err != nil {
		return nil, err
	}
	var out []Material
...
func (c *Catalog) enrichRow(row shopwrite.ParamRow, blob []byte, schema *shopwrite.ShopSchema, mtrl *mtrlContext) (*Row, error) {
	fields, err := shopwrite.DecodeRowFields(blob, row.DataOffset, schema)
	if err != nil {
		return nil, err
	}
	rid := int64(row.ID)
