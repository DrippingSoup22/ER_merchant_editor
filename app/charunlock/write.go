...
// acquisitions, see charunlock.BellBearing) reuse the exact same
...
// against, mirroring app/shopwrite.Apply's round-trip self-check: a bug
...
// the given character slot. A thin SetReleaseBatch wrapper for the common
// single-direction case (the CLI, and "Unlock all remaining").
func SetRelease(saveData []byte, charIndex int, rows []*catalog.Row, released bool) (int, error) {
	targets := make([]FlagTarget, len(rows))
	for i, r := range rows {
		targets[i] = FlagTarget{FlagID: r.UnlockFlag, Released: released, Label: fmt.Sprintf("row %d", r.RowID)}
	}
...
func Unlock(saveData []byte, charIndex int, rows []*catalog.Row) (int, error) {
...
// app/shopwrite.Apply's "never overwrite the input" convention). Returns
