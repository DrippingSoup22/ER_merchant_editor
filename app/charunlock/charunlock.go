...
// eventFlag_forRelease gate (app/catalog's Row.UnlockFlag) against a
// chosen character's event-flags bitfield (app/charflags,
// app/charslot), to answer "which merchant rows does this character
...
// slot. Rows with no gate (UnlockFlag == 0) are never included. A row
...
// whole batch.
func LockedRows(saveData []byte, charIndex int, rows []*catalog.Row) ([]*catalog.Row, error) {
	flags, err := eventFlags(saveData, charIndex)
	if err != nil {
		return nil, err
	}
	var locked []*catalog.Row
	for _, r := range rows {
		if r.UnlockFlag == 0 {
			continue
		}
		set, err := charflags.Get(flags, uint32(r.UnlockFlag))
		if err != nil {
			continue
		}
		if !set {
...
// whether it's currently released for the given character slot. Unlike
...
func LockStates(saveData []byte, charIndex int, rows []*catalog.Row) (map[int64]bool, error) {
	flags, err := eventFlags(saveData, charIndex)
	if err != nil {
		return nil, err
	}
...
// character slot. Rows with no gate are always reported unlocked.
func IsUnlocked(saveData []byte, charIndex int, row *catalog.Row) (bool, error) {
	if row.UnlockFlag == 0 {
		return true, nil
	}
	flags, err := eventFlags(saveData, charIndex)
	...
	return charflags.Get(flags, uint32(row.UnlockFlag))
}
