// Command charunlock is a dev-only CLI over app/charunlock, for scriptable
...
//
//	go run ./app/cmd/charunlock -save <in> -list-chars
//
// List a character's still-locked rows for one merchant (no write):
//
//	go run ./app/cmd/charunlock -save <in> -char 0 -merchant "Patches" -list-locked
//
// Unlock every currently-locked row for one merchant, for one character:
//
//	go run ./app/cmd/charunlock -save <in> -out <out> -char 0 -merchant "Patches"
//
// Unlock specific rows by ID instead:
//
//	go run ./app/cmd/charunlock -save <in> -out <out> -char 0 -rows 100057,100058
//
// Re-lock instead of unlock (either -merchant or -rows form):
//
//	go run ./app/cmd/charunlock -save <in> -out <out> -char 0 -rows 100057,100058 -lock
package main
...
func main() {
	...
	if *listChars {
		for _, ch := range charunlock.ListCharacters(data) {
	...
	if *listLocked {
		locked, err := charunlock.LockedRows(data, *charIndex, rows)
		if err != nil {
			fatal(err)
		}
	...
	released := !*lock
	n, err := charunlock.SetRelease(data, *charIndex, rows, released)
	if err != nil {
		fatal(err)
	}
